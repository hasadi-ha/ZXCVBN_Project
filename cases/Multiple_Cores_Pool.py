### WARINING ###
### Pool is still not operational in this situation
...
def runSearch(inputFile, currentIndex):
    ...
    for line in fileIn:
        ...
        try:
            ...
            # Add the location and reason for fail to errorLoc list
            errorLoc.append("***__ZXCVBN__ fail at location: " + str(count) + "***")
...
if __name__ == "__main__":
    ...
    for i in range(0, inputIndexAmount):
        inputIndexList.append([inputFileName, i])
    ...
    p = Pool(inputIndexAmount - 1)
    p.map(runSearch, inputIndexAmount)
